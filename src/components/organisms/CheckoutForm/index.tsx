...
const CheckoutForm = (): JSX.Element => {
  ...
  const options: Array<'e-Money' | 'Cash on Delivery'> = ['e-Money', 'Cash on Delivery'];
  const [checkedOption, setCheckedOption] = useState<'e-Money' | 'Cash on Delivery'>(
    options[0]
  );
  const [isDisabled, setIsDisabled] = useState(false);

  const handleChange = (value: 'e-Money' | 'Cash on Delivery') => {
  ...
  // Type assertion to bypass the complex type error
  const group = getRootProps() as Record<string, any>;
